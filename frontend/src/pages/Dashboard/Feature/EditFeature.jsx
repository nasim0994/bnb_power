...
export default function EditFeature() {
  ...
  const { data, isLoading } = useGetSingleFeatureQuery(id);
  const service = data?.data;

  const [editFeature, { isLoading: editLoading }] = useEditFeatureMutation();
  ...
  const handleEditFeature = async (e) => {
    ...
    const res = await editFeature({ id, formData });
    if (res?.data?.success) {
  ...
  return (
    <section className="bg-base-100 shadow rounded">
      ...
      <form onSubmit={handleEditFeature} className="p-4">
